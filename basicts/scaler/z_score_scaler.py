...
class ZScoreScaler(BaseScaler):
    ...
    def __init__(
        ...
        if norm_each_channel:
            try:
                mean_std = np.load(f"datasets/{dataset_name}/mean_std.npy")
                self.mean = torch.tensor(mean_std[:, [0]].T)
                self.std = torch.tensor(mean_std[:, [1]].T)
            except:
                print("No mean_std file")
    ...
    def transform(self, input_data: torch.Tensor) -> torch.Tensor:
        ...
        input_data[..., self.target_channel] = (
            input_data[..., self.target_channel] - self.mean.to(input_data)
        ) / self.std.to(input_data)
        return input_data
    ...
    def inverse_transform(self, input_data: torch.Tensor) -> torch.Tensor:
        ...
        input_data[..., self.target_channel] = input_data[
            ..., self.target_channel
        ] * self.std.to(input_data) + self.mean.to(input_data)
        return input_data
