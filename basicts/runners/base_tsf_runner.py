...
class BaseTimeSeriesForecastingRunner(BaseRunner):
    ...
    def postprocessing(self, input_data: Dict) -> Dict:
        ...
        if self.scaler is not None and self.scaler.rescale:
            input_data["prediction"] = self.scaler.inverse_transform(
            ...
            input_data["mean"] = self.scaler.mean
            input_data["std"] = self.scaler.std
    ...
    def val_iters(self, iter_index: int, data: Union[torch.Tensor, Tuple]):
        ...
        forward_return = self.postprocessing(forward_return)
        forward_return["mean"] = self.scaler.mean
        forward_return["std"] = self.scaler.std
        for metric_name, metric_func in self.metrics.items():
            metric_item = self.metric_forward(metric_func, forward_return)
    ...
    def test(
        ...
        returns_all["mean"] = self.scaler.mean
        returns_all["std"] = self.scaler.std
    ...
    def on_validating_end(self, train_epoch: Optional[int]):
        ...
        if train_epoch is not None:
            self.save_best_model(
                train_epoch, "val_" + self.target_metrics, greater_best=greater_best
            )
